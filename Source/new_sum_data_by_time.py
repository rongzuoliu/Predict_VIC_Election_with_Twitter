...
def archive_to_files(total):
    ...
    i = 0
    for elect, count in counts_by_dt.iteritems():
...
def main():
    ...
    year_views = db['_design/mapviews']['views'].keys()
    year_view_names = []
    for y in year_views:
        print y
        year_view_names.append('mapviews/'+y)
        print year_view_names
    for year_view in year_view_names:
        print year_view
        for row in db.view(year_view):
            id = row.id
            doc = db.get(id)
